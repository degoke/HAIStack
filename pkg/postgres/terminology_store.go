...
func (s *TerminologyStore) FindResource(ctx context.Context, scope, typ, url, ver string) (*store.TerminologyResourceRecord, error) {
	...
	err := s.exec.QueryRow(ctx, `SELECT scope_id,resource_type,resource_id,canonical_url,version,status,resource_json,content_mode,source_module FROM hai_terminology_resource WHERE scope_id=$1 AND resource_type=$2 AND canonical_url=$3 AND version=$4`, scope, typ, url, ver).Scan(&r.ScopeID, &r.ResourceType, &r.ResourceID, &r.CanonicalURL, &r.Version, &r.Status, &r.ResourceJSON, &r.ContentMode, &r.SourceModule)
...
func (s *TerminologyStore) PutResource(ctx context.Context, r store.TerminologyResourceRecord) error {
	_, err := s.exec.Exec(ctx, `INSERT INTO hai_terminology_resource(scope_id,resource_type,resource_id,canonical_url,version,status,resource_json,content_mode,source_module) VALUES($1,$2,$3,$4,$5,$6,$7::jsonb,$8,$9) ON CONFLICT(scope_id,resource_type,canonical_url,version) DO UPDATE SET resource_id=excluded.resource_id,status=excluded.status,resource_json=excluded.resource_json,content_mode=excluded.content_mode,source_module=excluded.source_module,updated_at=now()`, r.ScopeID, r.ResourceType, r.ResourceID, r.CanonicalURL, r.Version, r.Status, r.ResourceJSON, r.ContentMode, r.SourceModule)
	return err
}
func (s *TerminologyStore) DeleteResource(ctx context.Context, scope, typ, url, ver string) error {
	_, e := s.exec.Exec(ctx, `DELETE FROM hai_terminology_resource WHERE scope_id=$1 AND resource_type=$2 AND canonical_url=$3 AND version=$4`, scope, typ, url, ver)
	return e
}
func (s *TerminologyStore) ListResources(ctx context.Context, scope, typ string) ([]store.TerminologyResourceRecord, error) {
	rows, e := s.exec.Query(ctx, `SELECT scope_id,resource_type,resource_id,canonical_url,version,status,resource_json,content_mode,source_module FROM hai_terminology_resource WHERE scope_id=$1 AND ($2='' OR resource_type=$2) ORDER BY canonical_url,version`, scope, typ)
	if e != nil {
		return nil, e
	}
	defer rows.Close()
...
func (s *TerminologyStore) LookupConcept(ctx context.Context, scope, url, ver, code string) (*store.TerminologyConceptRecord, error) {
	q := `SELECT scope_id,system_url,system_version,code,display,definition,active,abstract,parent_code,COALESCE(properties_json::text,'{}'),COALESCE(designations_json::text,'{}') FROM hai_terminology_concept WHERE scope_id=$1 AND system_url=$2 AND system_version=$3 AND code=$4`
	args := []any{scope, url, ver, code}
	if ver == "" {
		q = `SELECT c.scope_id,c.system_url,c.system_version,c.code,c.display,c.definition,c.active,c.abstract,c.parent_code,COALESCE(c.properties_json::text,'{}'),COALESCE(c.designations_json::text,'{}') FROM hai_terminology_concept c LEFT JOIN hai_terminology_resource r ON r.scope_id=c.scope_id AND r.resource_type='CodeSystem' AND r.canonical_url=c.system_url AND r.version=c.system_version WHERE c.scope_id=$1 AND c.system_url=$2 AND c.code=$3 AND c.active AND COALESCE(r.status,'')!='retired' ORDER BY c.system_version DESC LIMIT 1`
...
func (s *TerminologyStore) ReplaceValueSet(ctx context.Context, r store.TerminologyValueSetRecord, ms []store.TerminologyExpansionMemberRecord) error {
	if _, e := s.exec.Exec(ctx, `INSERT INTO hai_terminology_valueset(scope_id,canonical_url,version,status,compose_json,expansion_json,expansion_timestamp,expansion_fingerprint) VALUES($1,$2,$3,$4,$5::jsonb,$6::jsonb,NULLIF($7,'')::timestamptz,$8) ON CONFLICT(scope_id,canonical_url,version) DO UPDATE SET status=excluded.status,compose_json=excluded.compose_json,expansion_json=excluded.expansion_json,expansion_timestamp=excluded.expansion_timestamp,expansion_fingerprint=excluded.expansion_fingerprint`, r.ScopeID, r.CanonicalURL, r.Version, r.Status, jsonOrEmpty(r.ComposeJSON), jsonOrEmpty(string(r.ExpansionJSON)), r.ExpansionTimestamp, r.ExpansionFingerprint); e != nil {
		return e
	}
	if _, e := s.exec.Exec(ctx, `DELETE FROM hai_terminology_expansion_member WHERE scope_id=$1 AND valueset_url=$2 AND valueset_version=$3`, r.ScopeID, r.CanonicalURL, r.Version); e != nil {
		return e
	}
	for _, m := range ms {
		if _, e := s.exec.Exec(ctx, `INSERT INTO hai_terminology_expansion_member(scope_id,valueset_url,valueset_version,system_url,system_version,code,display,inactive) VALUES($1,$2,$3,$4,$5,$6,$7,$8)`, r.ScopeID, r.CanonicalURL, r.Version, m.SystemURL, m.SystemVersion, m.Code, m.Display, m.Inactive); e != nil {
			return e
		}
	}
	return nil
...
func (s *TerminologyStore) GetValueSet(ctx context.Context, scope, url, ver string) (*store.TerminologyValueSetRecord, error) {
	q := `SELECT scope_id,canonical_url,version,status,compose_json::text,COALESCE(expansion_json::text,''),COALESCE(expansion_timestamp::text,''),expansion_fingerprint FROM hai_terminology_valueset WHERE scope_id=$1 AND canonical_url=$2 AND version=$3`
	args := []any{scope, url, ver}
	if ver == "" {
		q = `SELECT scope_id,canonical_url,version,status,compose_json::text,COALESCE(expansion_json::text,''),COALESCE(expansion_timestamp::text,''),expansion_fingerprint FROM hai_terminology_valueset WHERE scope_id=$1 AND canonical_url=$2 AND status!='retired' ORDER BY version DESC LIMIT 1`
		args = []any{scope, url}
	}
...
func (s *TerminologyStore) ListValueSetMembers(ctx context.Context, scope, url, ver string) ([]store.TerminologyExpansionMemberRecord, error) {
	rows, e := s.exec.Query(ctx, `SELECT scope_id,valueset_url,valueset_version,system_url,system_version,code,display,inactive FROM hai_terminology_expansion_member WHERE scope_id=$1 AND valueset_url=$2 AND valueset_version=$3`, scope, url, ver)
	if e != nil {
		return nil, e
	}
	defer rows.Close()
...
func (s *TerminologyStore) DeleteProjections(ctx context.Context, scope, typ, url, ver string) error {
	table := `hai_terminology_valueset`
	if typ == "CodeSystem" {
		table = `hai_terminology_concept`
	}
