...
func (s *Server) handleAuthorize(w http.ResponseWriter, r *http.Request) {
	...
	code := randomToken()
	s.tokens.saveCode(code, authCode{
		ClientID:    clientID,
...
func (s *Server) handleAuthorizationCode(w http.ResponseWriter, r *http.Request) {
	...
	entry, ok := s.tokens.consumeCode(code)
...
func (s *Server) handleClientCredentials(w http.ResponseWriter, r *http.Request) {
	...
	claims, _, err := s.backendAuth.ValidateBackendAssertion(assertion, smart.TokenValidateOptions{
		RequireIssuer:   true,
...
func (s *Server) handleRefreshToken(w http.ResponseWriter, r *http.Request) {
	...
	clientID := r.Form.Get("client_id")
	entry, ok := s.tokens.consumeRefresh(token)
...
func (s *Server) issueTokens(clientID, scope, patient, subject string) (map[string]any, error) {
	...
	refresh := randomToken()
	s.tokens.saveRefresh(refresh, refreshToken{
		ClientID:  clientID,
		Scope:     scope,
		Patient:   patient,
		Subject:   subject,
