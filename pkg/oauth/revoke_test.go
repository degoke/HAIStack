...
func TestOAuthServer_RevokeRejectsForeignAccessToken(t *testing.T) {
	mux := http.NewServeMux()
	srv := httptest.NewServer(mux)
	defer srv.Close()
	base := strings.TrimSuffix(srv.URL, "/")
	...
	foreignForm := url.Values{}
	foreignForm.Set("token", tokenResp.AccessToken)
	foreignForm.Set("token_type_hint", "access_token")
	foreignForm.Set("client_id", "other-client")
	foreignForm.Set("client_secret", "other-secret")
	foreignResp, err := http.PostForm(base+"/oauth/revoke", foreignForm)
	if err != nil {
	...
	if _, _, err := resolver(context.Background(), req); err != nil {
		t.Fatalf("foreign revoke should not invalidate access token: %v", err)
	}
}
