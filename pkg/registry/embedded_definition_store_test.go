...
func TestEmbeddedDefinitionStoreLoadsVersionedCanonicalURL(t *testing.T) {
	store := EmbeddedDefinitionStore()
	record, err := store.Get(context.Background(), "http://hl7.org/fhir/StructureDefinition/Patient|4.0.1", "")
	if err != nil {
		t.Fatal(err)
	}
	if record == nil || len(record.JSONData) == 0 {
		t.Fatal("expected versioned Patient StructureDefinition")
	}
}
