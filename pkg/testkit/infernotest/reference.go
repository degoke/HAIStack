...
func BuildReferenceHandler(ctx context.Context, baseURL string) (http.Handler, ReferenceMeta, func(), error) {
  ...
  "rules": [{
	...
	if err := oauthCfg.Clients.Register(oauth.Client{
		...
		Scopes:       []string{"patient/Patient.read", "patient/*.read", "launch/patient", "openid", "fhirUser", "offline_access"},
	}); err != nil {
		cleanup()
		return nil, ReferenceMeta{}, nil, err
	}
	...
	bearer := oauthSrv.BearerAuthConfig(adapter)
	...
	fhirHandler, err := hahttp.NewHandler(hahttp.Config{
		...
		AuthChecker:        smart.ScopePolicyAuthChecker{Engine: authEngine, Adapter: adapter},
	})
	if err != nil {
		cleanup()
		return nil, ReferenceMeta{}, nil, err
	...
	handler := hahttp.NewRootHandlerFromConfig(hahttp.RootConfig{
		FHIR:  fhirHandler,
		OAuth: oauthSrv.Handler(),
	})
