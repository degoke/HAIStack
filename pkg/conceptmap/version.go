...
func latestMapVersion(matches []Map) (Map, error) {
	if len(matches) == 0 {
		return Map{}, fmt.Errorf("no ConceptMap matches")
	}
	if len(matches) == 1 {
	...
	for i := 1; i < len(matches); i++ {
		...
		if !ok || !bestOK {
			return Map{}, fmt.Errorf("ambiguous ConceptMap versions for %s: non-semver version", best.URL)
		}
		if score > bestScore {
			best = candidate
			bestScore = score
	...
	if sameVersionCount > 1 {
		return Map{}, fmt.Errorf("ambiguous ConceptMap versions for %s", best.URL)
	}
	return best, nil
}
