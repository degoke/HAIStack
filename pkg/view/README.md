...
- Parquet export writes Apache Parquet binary (`application/vnd.apache.parquet`). Default `_parquetLayout=flat` streams flat ViewDefinition columns; `_parquetLayout=fhir` writes full Parquet-on-FHIR nested layouts (LIST/GROUP, choice types, extensions, `_primitive` wrappers, query annotations) from base StructureDefinitions via `pkg/parquetfhir`.
